...
const participantTableName = process.env.PARTICIPANT_AIRTABLE_TABLE_NAME || 'participants';
...
const participantFormHTML = (formId, error = '') => `
<!DOCTYPE html>
<html>
<head>
  <title>Campfire Forms</title>
...
app.get('/participant/:formId', (req, res) => {
  res.send(participantFormHTML(req.params.formId));
});

app.post('/participant/:formId', async (req, res) => {
  const { formId } = req.params;
  const { email } = req.body;
  ...
  const sanitizedEmail = email.trim().toLowerCase();
  const escapedEmail = sanitizedEmail.replace(/"/g, '\\"');
  const filterFormula = `FIND("${escapedEmail}", LOWER(ARRAYJOIN({email}, ",")))`;
  console.log(`[SEARCH] Table: ${participantTableName}, Filter: ${filterFormula}`);

  try {
    const records = await base(participantTableName)
      .select({
        filterByFormula: filterFormula,
        maxRecords: 1,
      })
    ...
    const record = records[0];
    const recordId = record.id;
    const participantEmailField = record.get('email');
    console.log(`[SEARCH] Match found - Record ID: ${recordId}, participant_email: "${participantEmailField}"`);
